...
from app.models.spo import (
    ...
    SPOStatusHistory, SPOAmendment, SPOAmendmentStatus, SPODeliverySchedule
)
from sqlalchemy.orm import selectinload
from app.schemas.spo import SPOCreate, SPOAcknowledgeReq, SPODeliveryScheduleCreate
...
class SPOService:
    ...
    async def create_draft(session: AsyncSession, workspace_id: uuid.UUID, spo_in: SPOCreate, current_user_id: uuid.UUID) -> SupplierPurchaseOrder:
        # Generate SPO number
        spo_number = f"SPO-{datetime.now().year}-000001" # Mocked for now, should query latest
    ...
    @staticmethod
    async def submit_for_approval(session: AsyncSession, spo_id: uuid.UUID, current_user_id: uuid.UUID) -> SupplierPurchaseOrder:
        spo = await session.get(SupplierPurchaseOrder, spo_id, options=[selectinload(SupplierPurchaseOrder.items)])
        if not spo or spo.status != SPOStatus.DRAFT:
    ...
    @staticmethod
    async def approve(session: AsyncSession, spo_id: uuid.UUID, current_user_id: uuid.UUID) -> SupplierPurchaseOrder:
        spo = await session.get(SupplierPurchaseOrder, spo_id, options=[selectinload(SupplierPurchaseOrder.items)])
        if not spo or spo.status != SPOStatus.PENDING_APPROVAL:
            raise HTTPException(status_code=400, detail="Invalid state transition")
    ...
    @staticmethod
    async def send(session: AsyncSession, spo_id: uuid.UUID, current_user_id: uuid.UUID) -> SupplierPurchaseOrder:
        spo = await session.get(SupplierPurchaseOrder, spo_id, options=[selectinload(SupplierPurchaseOrder.items)])
        if not spo or spo.status != SPOStatus.APPROVED:
            raise HTTPException(status_code=400, detail="Invalid state transition")
    ...
    @staticmethod
    async def acknowledge(session: AsyncSession, spo_id: uuid.UUID, ack_req: SPOAcknowledgeReq, current_user_id: uuid.UUID) -> SupplierPurchaseOrder:
        spo = await session.get(SupplierPurchaseOrder, spo_id, options=[selectinload(SupplierPurchaseOrder.items)])
        if not spo or spo.status not in (SPOStatus.SENT, SPOStatus.PARTIALLY_ACKNOWLEDGED):
            raise HTTPException(status_code=400, detail="Invalid state transition")
    ...
    @staticmethod
    async def cancel(session: AsyncSession, spo_id: uuid.UUID, reason: str, current_user_id: uuid.UUID) -> SupplierPurchaseOrder:
        spo = await session.get(SupplierPurchaseOrder, spo_id, options=[selectinload(SupplierPurchaseOrder.items)])
        if not spo or spo.status in (SPOStatus.CANCELLED, SPOStatus.CLOSED, SPOStatus.SHORT_CLOSED):
             raise HTTPException(status_code=400, detail="Invalid state transition")
