...
async def create_supplier_invoice(
    ...
    invoice = await supplier_invoice_service.create_supplier_invoice(session, current_user.workspace_id, invoice_in)
...
async def list_supplier_invoices(
    ...
    stmt = select(SupplierInvoice).where(SupplierInvoice.workspace_id == current_user.workspace_id)
...
async def get_supplier_invoice(
    ...
    stmt = select(SupplierInvoice).where(SupplierInvoice.id == id, SupplierInvoice.workspace_id == current_user.workspace_id)
...
@router.post("/{id}/submit-matching", response_model=SuccessResponse[SupplierInvoiceResponse])
async def submit_matching(
    id: uuid.UUID,
    session: AsyncSession = Depends(get_session),
    current_user: User = Depends(get_current_user)
):
    invoice = await supplier_invoice_service.submit_matching(session, id, current_user.workspace_id)
    return SuccessResponse(data=invoice)

@router.post("/{id}/resolve-discrepancy", response_model=SuccessResponse[SupplierInvoiceResponse])
async def resolve_discrepancy(
    ...
    invoice = await supplier_invoice_service.resolve_discrepancy(session, id, current_user.workspace_id, resolution)
    return SuccessResponse(data=invoice)
...
async def approve_invoice(
    ...
    invoice = await supplier_invoice_service.approve_invoice(session, id, current_user.workspace_id)
    return SuccessResponse(data=invoice)
