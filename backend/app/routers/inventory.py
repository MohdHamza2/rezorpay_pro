...
from app.models.inventory import Warehouse, WarehouseBin, InventoryLevel, InventoryTransaction, TransactionType
from app.schemas.inventory import WarehouseCreate, WarehouseResponse, WarehouseBinCreate, WarehouseBinResponse, InventoryLevelResponse, StockAdjustmentRequest
from app.schemas.common import SuccessResponse

router = APIRouter(prefix="/inventory", tags=["Inventory Management"])

@router.get("/warehouses", response_model=SuccessResponse[List[WarehouseResponse]])
async def list_warehouses(session: AsyncSession = Depends(get_session), workspace_id: uuid.UUID = Depends(get_current_workspace_id)):
    result = await session.execute(select(Warehouse).where(Warehouse.workspace_id == workspace_id))
    return SuccessResponse(data=result.scalars().all())

@router.post("/warehouses", response_model=SuccessResponse[WarehouseResponse])
async def create_warehouse(data: WarehouseCreate, session: AsyncSession = Depends(get_session), workspace_id: uuid.UUID = Depends(get_current_workspace_id)):
...
@router.post("/warehouses/{warehouse_id}/bins", response_model=SuccessResponse[WarehouseBinResponse], status_code=201)
...
@router.get("/warehouses/{warehouse_id}/bins", response_model=SuccessResponse[List[WarehouseBinResponse]])
...
async def get_inventory_levels(product_id: uuid.UUID = None, session: AsyncSession = Depends(get_session), workspace_id: uuid.UUID = Depends(get_current_workspace_id)):
...
async def adjust_stock(
    ...
    if not level:
        level = InventoryLevel(
            ...
            bin_id=data.bin_id,
            on_hand=0, reserved=0, damaged=0
        )
        session.add(level)
    ...
    if level.on_hand < 0:
        raise HTTPException(status_code=400, detail="Cannot have negative on-hand stock")
