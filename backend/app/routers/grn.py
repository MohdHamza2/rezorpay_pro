...
async def add_grn_item(
    ...
    grn_query = await session.execute(
        select(GoodsReceiptNote)
        .options(selectinload(GoodsReceiptNote.items))
        .where(GoodsReceiptNote.id == id)
    )
    return SuccessResponse(data=grn_query.scalar_one())


@router.post(
    "/grns/{id}/stage-for-inspection", response_model=SuccessResponse[GRNResponse]
)
async def stage_for_inspection(
    ...
    await GRNService.stage_for_inspection(session, workspace_id, id)
    await session.commit()
...
@router.post(
    "/grns/{id}/items/{item_id}/disposition",
    response_model=SuccessResponse[GRNResponse],
)
async def record_disposition(
    ...
    await GRNService.record_disposition(
        session, workspace_id, id, item_id, user.id, data
    )
    await session.commit()
...
async def get_grn_reconciliation(
    ...
    grn_query = await session.execute(
        select(GoodsReceiptNote)
        .options(selectinload(GoodsReceiptNote.items))
        .where(GoodsReceiptNote.id == id, GoodsReceiptNote.workspace_id == workspace_id)
    )
...
async def list_grns(
    ...
    query = (
        select(GoodsReceiptNote)
        .options(selectinload(GoodsReceiptNote.items))
        .where(GoodsReceiptNote.workspace_id == workspace_id)
    )
    if status:
...
async def list_spo_grns(
    ...
    result = await session.execute(
        ...
        .options(selectinload(GoodsReceiptNote.items))
        .where(
            GoodsReceiptNote.workspace_id == workspace_id, GoodsReceiptNote.spo_id == id
        )
