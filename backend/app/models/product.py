...
from sqlalchemy import Column, DateTime, String, Text, Numeric, Boolean, UniqueConstraint
...
class Product(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "internal_sku", name="uq_workspace_internal_sku"),
    )
    ...
    brand_id: Optional[uuid.UUID] = Field(default=None, foreign_key="brands.id", index=True)
    ...
    tax_rate: Optional[Decimal] = Field(default=None, sa_column=Column(Numeric(5, 2), nullable=True))
    reorder_level: Optional[Decimal] = Field(default=None, sa_column=Column(Numeric(12, 2), nullable=True))
...
class ProductIdentifier(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "type", "value", name="uq_product_identifier_type_value"),
    )
...
class ProductUOMConversion(SQLModel, table=True):
    ...
    conversion_factor: Decimal = Field(sa_column=Column(Numeric(14, 6), nullable=False)) # e.g. 1 BOX = 10 PCS
...
class ProductPrice(SQLModel, table=True):
    ...
    price_type: str = Field(max_length=50) # e.g. DEFAULT_SALES, TIER_1, CUSTOMER_SPECIFIC
    ...
    min_quantity: Optional[Decimal] = Field(default=None, sa_column=Column(Numeric(12, 2)))
