...
class Supplier(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "supplier_code", name="uq_workspace_supplier_code"),
    )
...
class SupplierDocument(SQLModel, table=True):
    ...
    document_type: str = Field(max_length=50) # e.g. TRADE_LICENSE, VAT_CERTIFICATE, CONTRACT
    document_url: str = Field(max_length=1000)
    expiry_date: Optional[datetime] = Field(default=None, sa_column=Column(DateTime(timezone=True)))
...
class SupplierProduct(SQLModel, table=True):
    ...
    moq: Optional[Decimal] = Field(default=None, sa_column=Column(Numeric(12, 2))) # Minimum Order Quantity
