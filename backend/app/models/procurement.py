...
from sqlalchemy import Column, DateTime, Date, Numeric, String, UniqueConstraint, Text
...
class ProcurementRequest(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "request_number", name="uq_workspace_pr_number"),
    )
...
class ProcurementRequestItem(SQLModel, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    request_id: uuid.UUID = Field(foreign_key="procurement_requests.id", nullable=False, index=True)
    ...
    requested_quantity: Decimal = Field(sa_column=Column(Numeric(12, 2), nullable=False))
    approved_quantity: Decimal = Field(default=0, sa_column=Column(Numeric(12, 2), nullable=False))
    ordered_quantity: Decimal = Field(default=0, sa_column=Column(Numeric(12, 2), nullable=False))
    cancelled_quantity: Decimal = Field(default=0, sa_column=Column(Numeric(12, 2), nullable=False))
    received_quantity: Decimal = Field(default=0, sa_column=Column(Numeric(12, 2), nullable=False))
