...
class GoodsReceiptNote(SQLModel, table=True):
    ...
    spo_id: Optional[uuid.UUID] = Field(
        default=None, foreign_key="supplier_purchase_orders.id", index=True
    )
    ...
    status: GRNStatus = Field(
        sa_column=Column(
            SAEnum(GRNStatus, name="grnstatus"), default=GRNStatus.DRAFT, nullable=False
        )
    )
...
class GRNItem(SQLModel, table=True):
    ...
    spo_item_id: uuid.UUID = Field(
        foreign_key="supplier_purchase_order_items.id", index=True
    )
    spo_delivery_schedule_id: Optional[uuid.UUID] = Field(
        default=None, foreign_key="spo_delivery_schedules.id"
    )
    ...
    quantity_ordered_snapshot: Decimal = Field(
        default=Decimal(0), max_digits=12, decimal_places=4
    )
    quantity_confirmed_snapshot: Decimal = Field(
        default=Decimal(0), max_digits=12, decimal_places=4
    )
