...
from sqlalchemy import Column, DateTime, Date, Numeric, String, UniqueConstraint, Text, Boolean, Integer
...
class SupplierPurchaseOrder(SQLModel, table=True):
    ...
    procurement_request_id: Optional[uuid.UUID] = Field(default=None, foreign_key="procurement_requests.id")
    
    procurement_method: ProcurementMethod = Field(nullable=False)
    single_source_justification: Optional[str] = Field(default=None, sa_column=Column(Text))
    ...
    quantity_confirmed_total: Decimal = Field(default=0, sa_column=Column(Numeric(12, 4)))
    quantity_backordered_total: Decimal = Field(default=0, sa_column=Column(Numeric(12, 4)))
...
class SupplierPurchaseOrderItem(SQLModel, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    spo_id: uuid.UUID = Field(foreign_key="supplier_purchase_orders.id", nullable=False, index=True)
    
    line_number: int = Field(nullable=False)
    
    rfq_award_line_id: Optional[uuid.UUID] = Field(default=None, foreign_key="rfq_award_lines.id")
    procurement_request_item_id: Optional[uuid.UUID] = Field(default=None, foreign_key="procurement_request_items.id")
    ...
    quantity_damaged_rejected: Decimal = Field(default=0, sa_column=Column(Numeric(12, 4)))
...
class SPODeliverySchedule(SQLModel, table=True):
    ...
    delayed_flagged_at: Optional[datetime] = Field(default=None, sa_column=Column(DateTime(timezone=True)))
...
class SPOAmendment(SQLModel, table=True):
    ...
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), sa_column=Column(DateTime(timezone=True)))
    applied_at: Optional[datetime] = Field(default=None, sa_column=Column(DateTime(timezone=True)))
...
class SPOAmendmentLine(SQLModel, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    amendment_id: uuid.UUID = Field(foreign_key="spo_amendments.id", nullable=False, index=True)
    spo_item_id: uuid.UUID = Field(foreign_key="supplier_purchase_order_items.id", nullable=False)
...
class SPOStatusHistory(SQLModel, table=True):
    ...
    occurred_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), sa_column=Column(DateTime(timezone=True)))
