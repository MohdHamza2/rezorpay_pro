...
from sqlalchemy import Column, DateTime, Numeric, String, UniqueConstraint, Text, Boolean, Integer
...
class RFQ(SQLModel, table=True):
    ...
    evaluation_criteria: RFQEvalCriteria = Field(default=RFQEvalCriteria.LOWEST_LANDED_COST)
    
    deadline: datetime = Field(sa_column=Column(DateTime(timezone=True), nullable=False))
    sealed_until: Optional[datetime] = Field(default=None, sa_column=Column(DateTime(timezone=True)))
...
class RFQItemSource(SQLModel, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    rfq_item_id: uuid.UUID = Field(foreign_key="rfq_items.id", nullable=False, index=True)
    procurement_request_item_id: uuid.UUID = Field(foreign_key="procurement_request_items.id", nullable=False)
    quantity_from_source: Decimal = Field(sa_column=Column(Numeric(12, 2), nullable=False))
...
class SupplierRFQResponse(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("rfq_id", "supplier_id", "revision_number", name="uq_rfq_supplier_revision"),
    )
    ...
    normalized_total: Optional[Decimal] = Field(default=None, sa_column=Column(Numeric(12, 2)))
...
class SupplierQuoteItem(SQLModel, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    response_id: uuid.UUID = Field(foreign_key="supplier_rfq_responses.id", nullable=False, index=True)
    ...
    alternate_product_id: Optional[uuid.UUID] = Field(default=None, foreign_key="products.id")
...
class RFQAward(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "award_number", name="uq_workspace_award_number"),
    )
...
class RFQAwardLine(SQLModel, table=True):
    ...
    quote_item_id: uuid.UUID = Field(foreign_key="supplier_quote_items.id", nullable=False)
