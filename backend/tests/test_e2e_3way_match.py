...
def test_3way_match_engine():
    ...
    # 3. Create UOM
    uom_r = client.post(
        "/api/v1/products/uom", headers=headers, json={"name": "Pieces", "code": "PCS"}
    )
    ...
    # 4. Create Warehouse
    warehouse_r = client.post(
        "/api/v1/inventory/warehouses",
        headers=headers,
        json={"name": "Main Warehouse", "code": "MAIN-01", "address": "Dubai"},
    )
    ...
    disp1 = {
        ...
        "quantity_damaged": 20,
        "quantity_rejected": 0,
        "damage_reason": "Bent surfaces on 20 units",
    }
    disp_r = client.post(
    ...
    approve_fail = client.post(
        f"/api/v1/supplier-invoices/{inv_id}/approve", headers=headers
    )
    ...
    assert (
        resolved_inv["three_way_match_notes"]
        == "Approved paying full invoice, supplier is sending replacement 20 next week."
    )
